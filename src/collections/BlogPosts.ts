...
const BlogPosts: CollectionConfig = {
  ...
  admin: {
    ...
    description: 'Blog articles and regulatory reference content. Regulatory posts require admin approval before they can publish.',
    preview: (doc) =>
  ...
  hooks: {
    beforeChange: [
      ...
      // Throws ValidationError (not plain Error) so the admin shows the message inline on the
      // Review Status field and in the toast — not a generic "Something went wrong" banner.
        ...
        if (isPublishing && isRegulatory) {
          ...
          if (reviewStatus !== 'approved') {
            throw new ValidationError({
              errors: [
                {
                  // Attach to the reviewStatus field so the editor sees the red highlight
                  // AND the message in the publish-blocked toast.
                  path: 'reviewStatus',
                  message:
                    "This regulatory post can't be published until a qualified compliance " +
                    'professional sets Review Status to "Approved". ' +
                    'Update the Review Status field in the sidebar and try again.',
                },
              ],
            })
          }
